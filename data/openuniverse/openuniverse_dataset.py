...
def _token_fields(window):
    ...
    magnitude = np.where(
        token_type == 'd', window['mag_observed'].to_numpy(dtype=float),
...
def compute_normalization(long_df):
    ...
    return {
        'mag_mean': float(magnitude.mean()), 'mag_std': float(magnitude.std() + 1e-8),
        'sigma_mean': float(sigma.mean()), 'sigma_std': float(sigma.std() + 1e-8),
    }
...
class EarlyWindowDataset(Dataset):
    ...
    def __init__(self, long_df, normalization):
        ...
        self.objects = []
        for object_id, window in long_df.groupby('object_id', sort=False):
    ...
    def _encode_object(self, window):
        ...
        magnitude = np.where(
            magnitude_mask,
            (fields['magnitude'] - normalization['mag_mean']) / normalization['mag_std'], 0.0,
        )
        sigma_magnitude = np.where(
            sigma_mask,
            (fields['sigma_magnitude'] - normalization['sigma_mean']) / normalization['sigma_std'], 0.0,
        )
...
def split_objects(long_df, fractions=(0.70, 0.15, 0.15), random_seed=42):
    ...
    return {name: long_df[long_df['object_id'].isin(ids)].copy()
            for name, ids in split_ids.items()}


def build_dataloaders(long_df, batch_size=32, fractions=(0.70, 0.15, 0.15), random_seed=42,
                      num_workers=0):
    ...
    for name, split_df in splits.items():
        ...
        loaders[name] = DataLoader(
            ...
            num_workers=num_workers, collate_fn=collate_token_windows,
        )
