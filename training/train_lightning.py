...
from openuniverse_data import GROUP_ORDER, build_dataloaders
from torchmetrics import MeanMetric
from torchmetrics.classification import MulticlassAccuracy

NUM_CLASSES = len(GROUP_ORDER)
...
class LitKilonova(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        cosine = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=max(self.hparams.max_epochs - warmup_epochs, 1),
...
def train(
    ...
    print(
        f"best single val_acc_noz: {checkpoint_callback.best_model_score:.4f}  ->  "
    ...
    soup_model, individual_scores = build_model_soup(checkpoint_callback.best_k_models, weights, lit_model)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    soup_score, soup_per_regime = evaluate_noz(soup_model, regime_loaders, device)
    print(
        f"top-{len(individual_scores)} individual val_acc_noz (checkpointed): "
        f"{[round(s, 4) for s in individual_scores]}"
    )
    print(f"souped val_acc_noz: {soup_score:.4f}  (per noz-regime: {[round(s, 4) for s in soup_per_regime]})")
    ...
    for regime in regime_loaders:
        ...
        with torch.no_grad():
            for batch in regime["loader"]:
                model_batch = {
                    key: value.to(device) for key, value in batch.items() if key in MODEL_INPUT_KEYS
                }
...
def parse_arguments():
    ...
    parser.add_argument(
        "--patience", type=int, default=25, help="early-stopping patience on val_acc (epochs)"
    )
    parser.add_argument("--d-model", type=int, default=192)
    parser.add_argument("--num-heads", type=int, default=6)
    parser.add_argument("--num-layers", type=int, default=6)
    parser.add_argument(
        "--d-feedforward", type=int, default=768, help="FFN width; standard transformer ratio is 4x d_model"
    )
    parser.add_argument("--dropout", type=float, default=0.1)
    parser.add_argument(
        "--warmup-epochs", type=int, default=5, help="linear LR warmup length before cosine decay (epochs)"
    )
